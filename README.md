## Arodi Emmanuel Haro

- Software Engineer
- Automation & Data Specialist
- Digital Publisher

## 🛠️ Technologies & Tools

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)

## What I Do

- Web scraping and data extraction pipelines
- API development and third-party integrations
- Automation workflows for publishing and distribution
- Full-stack applications with React and Spring Boot
- Digital book production: translation, typesetting, and distribution
- Exploring generative AI (TTS) with local models

## 📊 GitHub Stats

![](https://github-readme-stats.vercel.app/api?username=arodoo&show_icons=true&theme=gruvbox&hide_border=true)
![](https://github-readme-stats.vercel.app/api/top-langs/?username=arodoo&layout=compact&theme=gruvbox&hide_border=true)

📫 **Let's connect:**
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:haro.trabajo@gmail.com)
